...
FILE *file, *f1;
int f = 0, m = 0;
char rtrv[100], wrtn[100], match[100];
...
void saved_data() {
    ...
    while ((fread(&std, sizeof(std), 1, file)) == 1) {
        ...
        *temp = std;
        ...
        if (strcmp(temp->gender, "F") == 0 || strcmp(temp->gender, "f") == 0) f++;
        if (strcmp(temp->gender, "M") == 0 || strcmp(temp->gender, "m") == 0) m++;
...
void intro() {
    while (1) {
        ...
        printf("\t\t\t\t\tSTUDENT MANAGEMENT SYSTEM\n");
        printf("\t\t\t\t*-------------------------------------*\n\n\n");
        if (k == 1) printf("\t\t\t\t\tWrong password!! ,try again\n");
        if (p == 1) printf("\t\t\t\t\tIncorrect Username!! ,try again\n");
        ...
        do {
            password[l] = getch();
            if (password[l] != '\r') printf("*");
            l++;
        } while (password[l - 1] != '\r');
        password[l - 1] = '\0';
        if (strcmp(username, "wub") == 0 && strcmp(password, rtrv) == 0) {
            system("cls");
            printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
            ...
            if (strcmp(password, rtrv) != 0) k = 1;
            if (strcmp(username, "wub") != 0) p = 1;
        }
    }
}
...
int main() {
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 5: modify_student_data(); break;
            case 6: change_password(); break;
            case 7: system("cls"); exit(0);
            default: printf("Wrong Input.. END!!\n"); break;
        }
    }
    return 0;
}
